...
export const CreateOrder: React.FC = () => {
  ...
  const generateInvoiceId = async () => {
    ...
    try {
      // Get all orders for today to find the highest sequence number
      ...
      if (todayOrders && todayOrders.length > 0) {
        // Extract sequence number from last invoice (e.g., "251123-005" -> 5)
  ...
  const handlePlaceOrder = async () => {
    ...
    try {
      ...
      toast.success(`Order Placed! Invoice #${invoiceId} - ₹${finalTotal.toFixed(2)}`);
      setCart([]);
      setSelectedCompany('');
      setOrderDiscountPct(0);
    } catch (e) {
  ...
  const discountAmount = (subtotalAmount * orderDiscountPct) / 100; // Calculate amount from percentage
    ...
    <div className="flex flex-col lg:flex-row gap-6 lg:h-[calc(100vh-7rem)] h-auto pb-6">
      ...
      <Modal isOpen={isAddCustomerOpen} onClose={() => setAddCustomerOpen(false)} title="Quick Add Customer">
        <form onSubmit={(e) => { e.preventDefault(); handleAddCustomer(); }} className="space-y-4">
          <Input label="Shop Name" value={newCustomerName} onChange={e => setNewCustomerName(e.target.value)} placeholder="e.g. New General Store" error={validationErrors.name} />
          <div className="grid grid-cols-2 gap-4">
            <Input label="Phone" value={newCustomerPhone} onChange={e => setNewCustomerPhone(e.target.value)} placeholder="98..." error={validationErrors.phone} />
            <Input label="Route" value={newCustomerRoute} onChange={e => setNewCustomerRoute(e.target.value)} placeholder="Route Name" error={validationErrors.routeName} />
          </div>
          <Input label="PAN Number" value={newCustomerPan} onChange={e => setNewCustomerPan(e.target.value)} placeholder="Optional" error={validationErrors.panNumber} />
          ...
          <div className="flex justify-end gap-3 mt-6 pt-2">
            <Button variant="outline" onClick={() => setAddCustomerOpen(false)} type="button">Cancel</Button>
